...
MAKE_HOOK(CEconItemSchema_GetItemDefinition, Memory::RelToAbs(Signatures::CEconItemSchema_GetItemDefinition.Get()),
	CEconItemDefinition_t *, __fastcall, void *ecx, void *edx, int iItemIndex)
{
	if (CFG::Misc_Equip_Region_Unlock)
	{
		auto item{ CALL_ORIGINAL(ecx, edx, iItemIndex) };
	...
	return CALL_ORIGINAL(ecx, edx, iItemIndex);
}